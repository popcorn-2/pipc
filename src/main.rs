...
fn main() {
    let cli = command!()
        ...
        .arg(Arg::new("files").num_args(1..).trailing_var_arg(true))
        .get_matches();

    let input_files = cli.get_many::<String>("files").unwrap().collect::<Vec<_>>();
    let language = cli.get_one::<OutputFormat>("language").unwrap();
    let _format = cli.get_one::<OutputTy>("format").unwrap();
    ...
    for file in &input_files {
        match process_file(file, *language, output) {
            Err(e) => {
                eprintln!("{e}");
                error_count += 1;
            }
